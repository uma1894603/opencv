    ...
    for( i = 0; i < height; i++ )
    {
        ...
        if( cn == 1 )
        {
            ...
            for (; j < width; j++)
            {
                ...
                dptr[j] = (uchar)lrint(sum[j] / wsum[j]);
            }
        }
        else
        {
            ...
            for(; j < width; j++)
            {
                ndsrvp_assert(fabs(wsum[j]) > 0);
                wsum[j] = 1.f / wsum[j];
                *(dptr++) = (uchar)lrint(sum_b[j] * wsum[j]);
                *(dptr++) = (uchar)lrint(sum_g[j] * wsum[j]);
                *(dptr++) = (uchar)lrint(sum_r[j] * wsum[j]);
            }
        }
    }
}
    ...
    int cn, int d, double sigma_color, double sigma_space, int border_type)
{
    ...
    if( d <= 0 )
        radius = (int)lrint(sigma_space * 1.5);
    else
        radius = d / 2;
